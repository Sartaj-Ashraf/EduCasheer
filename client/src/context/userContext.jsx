...
export const UserProvider = ({ children }) => {
  ...
  const login = async (userData) => {
    try {
      ...
      const { data } = await customFetch.post("/users/login", userData);
      console.log({ user: data.message.user });

      // Store user data in localStorage

      if (data.message.user) {
        localStorage.setItem("Edu-user", JSON.stringify(data.message.user));
        localStorage.setItem("isAuthenticated", "true");
      }

      dispatch({ type: "SET_USER", payload: data.message.user });
  ...
  const logout = async () => {
    try {
      await customFetch.delete("/users/logout");
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem("Edu-user");
  ...
  const updateUser = async (updates) => {
    try {
      ...
      if (data.user) {
        ...
        localStorage.setItem("Edu-user", JSON.stringify(updatedUser));
